# SOP: Code Security Analysis (ShivaAuditor skill)  v3.0 (Neural Evolution)

**Trigger Command (Análise Oficial):** `ShivaAuditor [Project Path]`
...
13. **JWT Algorithm Confusion Attack:** *(Adicionado - automelhorar v1.4)*
    - Protocolo completo de análise de tokens JWT encontrados:
      ...
      3. **Verificar RS256HS256 downgrade:** se o servidor assina com RSA privada mas a verificação não força `algorithms=["RS256"]`, o atacante pega a chave pública (frequentemente disponível) e assina com HMAC usando ela como segredo.
...
### Fase 4.5  Limpeza Pós-Scan (OBRIGATÓRIA) *(Hardened v3.1)*
- **Após a conclusão de toda atividade SAST/DAST**, o motor DEVE executar a limpeza do diretório `.tmp/`.
- **Segurança do Caminho:** Use sempre o caminho absoluto baseado na raiz do projeto para evitar deleção acidental em diretórios incorretos (`Confused Deputy`).
- Comando Seguro:
  - **PowerShell:** `Get-ChildItem -Path "$ProjectRoot\.tmp\*" -Recurse | Remove-Item -Force`
  - **Bash:** `rm -rf "$(pwd)/.tmp/"*`
- **Validação:** Após o comando, execute `list_dir` em `.tmp/` para confirmar que está vazio.

42. **OpSec & Artifact Integrity:** *(Adicionado - automelhorar v3.1)*
    - Manter a integridade do ambiente de auditoria é parte da função.
    - **Leitura de Logs de Ferramentas:** Inspecionar `reports/` e logs de ferramentas em busca de segredos que foram capturados "sem querer" durante o scan e higienizá-los.
    - **Isolamento de Credenciais:** Nunca salvar credenciais reais do usuário em arquivos do repositório, mesmo que sejam para testes automáticos. Usar variáveis de ambiente injetadas no runtime.
...
#### RELATÓRIO TÉCNICO ÚNICO  `reports/ShivaAuditor_[project].md`
...
|---|---|
| 🔴 **Crítico** | RCE não-autenticado, SQLi com dump total, LFIRCE Chain |
...
Quando o usuário fornecer a URL de um alvo ativo (ex: `http://localhost/...`), o motor ShivaAi ganha prerrogativa total para fundir suas descobertas estáticas (SAST) em validações dinâmicas (DAST) ativas:
